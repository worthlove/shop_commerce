  ...
  <div style='width:100%; height:92%; padding-top:20px'>
    ...
    <el-card class='box-card'>
      <el-row :gutter='20'>
        ...
        <el-col :span='4'>
          <el-button type='primary' @click='addShowDialog =true'>添加用户</el-button>
        </el-col>
      </el-row>
      <!-- 用户列表区域 -->
      <el-table :data='userList' border stripe>
        ...
        <el-table-column label='状态'>
          ...
          <template v-slot='scope'>
            <el-switch v-model='scope.row.mg_state' @change='userStateChangeEvent(scope.row)'></el-switch>
          </template>
        </el-table-column>
        <el-table-column label='操作' width='300px'>
          <template v-slot='scope'>
            ...
            <el-tooltip :enterable='false' content='修改' effect='dark' placement='top'>
              <el-button icon='el-icon-edit' size='mini' type='primary' @click='editFn(scope.row.id)'>修改</el-button>
            </el-tooltip>
            <!-- 删除按钮 -->
            <el-tooltip :enterable='false' content='删除' effect='dark' placement='top'>
              <el-button icon='el-icon-delete' size='mini' type='danger' @click='deleteFn(scope.row.id)'>删除</el-button>
            </el-tooltip>
            <!-- 分配角色按钮 -->
            <el-tooltip :enterable='false' content='分配角色' effect='dark' placement='top'>
              <el-button icon='el-icon-setting' size='mini' type='warning'>分配角色</el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    >
      <!-- 内容主体区域 -->
      <el-form ref='ruleFormRef' :model='addFromData' :rules='addFromRules' label-width='70px'>
    ...
    >
      <!-- 内容主体区域 -->
      <el-form ref='editFormRef' :model='editFromData' :rules='editFromRules' label-width='70px'>
    ...
    editConfirmFn() {
      this.$refs.editFormRef.validate(async valid => {
        ...
        if (!valid) {
          ...
          // 发起编辑用户信息列表的数据请求
          const { data: res } = await this.$http.put('users/' + this.editFromData.id, {
